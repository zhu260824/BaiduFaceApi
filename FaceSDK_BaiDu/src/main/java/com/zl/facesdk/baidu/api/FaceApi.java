package com.zl.facesdk.baidu.api;
...
public class FaceApi {
    ...
    public boolean addFeature(Feature feature) {
        if (null == feature || TextUtils.isEmpty(feature.getGroupId()) || TextUtils.isEmpty(feature.getUserId()) || null == feature.getFeature() || feature.getFeature().length < 1) {
            return false;
        }
        Pattern pattern = Pattern.compile("^[0-9a-zA-Z_-]{1,}$");
    ...
    public IdentifyRet identity(int[] argbData, int rows, int cols, int[] landmarks, String groupId) {
        ...
        byte[] imageFrameFeature = new byte[2048];
        FaceSDKManager.getInstance().getFaceFeature().extractFeature(argbData, rows, cols, imageFrameFeature, landmarks);
        String userIdOfMaxScore = "";
        float identifyScore = 0;
        Iterator iterator = userId2Feature.entrySet().iterator();
        while (iterator.hasNext()) {
            Map.Entry<String, byte[]> entry = (Map.Entry<String, byte[]>) iterator.next();
            byte[] feature = entry.getValue();
            final float score = FaceSDKManager.getInstance().getFaceFeature().getFaceFeatureDistance(feature, imageFrameFeature);
            if (score > identifyScore) {
                identifyScore = score;
                userIdOfMaxScore = entry.getKey();
            }
    ...
    public void reloadFacesFromDB(String groupId) {
        if (TextUtils.isEmpty(groupId)) {
            return;
        }
        group2Facesets.clear();
        group2Facesets = new HashMap<>();
        List<Feature> featureList = DBManager.getInstance().queryFeatureByGroupId(groupId);
        HashMap<String, byte[]> userId2Feature = new HashMap<>();
        for (Feature feature : featureList) {
            userId2Feature.put(feature.getUserId(), feature.getFeature());

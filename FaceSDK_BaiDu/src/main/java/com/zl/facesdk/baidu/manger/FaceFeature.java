...
//            faceRecognize.initModel(FaceSDK.RecognizeType.RECOGNIZE_ID_PHOTO);
    ...
    public int extractIdCardFeature(int[] argb, int height, int width, int minfaceSize, byte[] feature) {
        ...
        int ret = faceRecognize.extractFeature(argb, height, width, FaceSDK.ImgType.ARGB, minfaceSize, feature, FaceSDK.RecognizeType.RECOGNIZE_ID_PHOTO);
        return ret;
    }
    ...
    public int extractFeatureWithDetect(int[] argb, int height, int width, byte[] feature, int[] landmarks) {
        ...
        int ret = faceRecognize.extractFeature(argb, height, width, FaceSDK.ImgType.ARGB, feature, landmarks, FaceSDK.RecognizeType.RECOGNIZE_ID_PHOTO);
        return ret;
    }
    ...
    public int faceFeature(ARGBImg argbImg, byte[] feature, int minFaceSize) {
        return faceRecognize.extractFeature(argbImg.data, argbImg.height, argbImg.width, FaceSDK.ImgType.ARGB, minFaceSize, feature, FaceSDK.RecognizeType.RECOGNIZE_LIVE);
    }


    public int faceFeatureForIDPhoto(ARGBImg argbImg, byte[] feature, int minFaceSize) {
        return faceRecognize.extractFeature(argbImg.data, argbImg.height, argbImg.width, FaceSDK.ImgType.ARGB, minFaceSize, feature, FaceSDK.RecognizeType.RECOGNIZE_ID_PHOTO);
    }
    ...
    public int faceFeature(ARGBImg argbImg, byte[] feature) {
        ...
        if (faceInfos != null && faceInfos.length > 0) {
            ...
            if (faceInfo != null && (ret != FaceDetector.NO_FACE_DETECTED && ret != FaceDetector.UNKNOW_TYPE)) {
                return faceRecognize.extractFeature(argbImg.data, argbImg.height, argbImg.width, FaceSDK.ImgType.ARGB, feature, faceInfo.landmarks, FaceSDK.RecognizeType.RECOGNIZE_LIVE);
            }
        }
        FaceSDKManager.getInstance().getFaceDetector().clearTrackedFaces();
        return ret;

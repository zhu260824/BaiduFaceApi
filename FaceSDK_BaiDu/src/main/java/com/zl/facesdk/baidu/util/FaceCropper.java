 ...
 */
package com.zl.facesdk.baidu.util;
...
public class FaceCropper {
    ...
    public static Rect getFaceRect(FaceInfo faceInfo, int fWidth, int fHeight) {
        int[] points = new int[8];
        Rect rect = new Rect();
        faceInfo.getRectPoints(points);
        int left = points[2];
        ...
        rect.right = (left + width) > fWidth ? fWidth : (left + width);
        rect.bottom = (top + height) > fHeight ? fHeight : (top + height);
        return rect;
    }

...
public class FaceDetector {
    ...
    public int detect(int[] argb, int width, int height) {
        if (null == mFaceTracker || initStatus != FaceSDKManager.SDK_INITED) {
            return UNKNOW_TYPE;
        }
        ...
        return mFaceTracker.prepare_max_face_data_for_verify(argb, height, width, FaceSDK.ImgType.ARGB.ordinal(), FaceTracker.ActionType.RECOGNIZE.ordinal());
    }
    ...
    public void setEulerAngleThreshold(int yaw, int roll, int pitch) {
        ...
        if (mFaceTracker != null) {
            mFaceTracker.set_eulur_angle_thr(pitch, yaw, roll);
        }
    }
